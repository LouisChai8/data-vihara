...
const roleStyle = {
  ...
  Viewer: {
    bg: "rgba(160,160,170,0.12)",
    color: "#9ca3af",
    border: "rgba(160,160,170,0.25)",
  },
};
...
function getWarna(id) {
  return warnaList[(id - 1) % warnaList.length];
}
...
function DetailModal({ anggota, onClose, onDelete, onEdit }) {
  ...
  const handleDelete = async () => {
    if (!confirm(`Hapus anggota "${anggota.nama_lengkap}"?`)) return;
    setDeleting(true);
    await onDelete(anggota.id);
    setDeleting(false);
    onClose();
  };
      ...
      >
            ...
            >
              ...
              <span
                style={{
                  fontSize: "11px",
                  fontWeight: "500",
                  color: anggota.status === "Aktif" ? "#7ec8a0" : "#c87e7e",
                  background:
                    anggota.status === "Aktif"
                      ? "rgba(126,200,160,0.1)"
                      : "rgba(200,126,126,0.1)",
                  border: `1px solid ${anggota.status === "Aktif" ? "rgba(126,200,160,0.25)" : "rgba(200,126,126,0.25)"}`,
                  padding: "2px 10px",
                  borderRadius: "20px",
                }}
              >
        ...
        <div style={{ padding: "24px 28px 28px" }}>
          <Section icon={UserIcon} title="Informasi Pribadi">
            ...
            <Field
              label="Tanggal Lahir"
              value={formatDate(anggota.tanggal_lahir)}
            />
          ...
          <Section icon={BookOpen} title="Informasi Keagamaan">
            <Field
              ...
              value={formatDate(anggota.tanggal_baptis)}
            />
            <Field
              label="Chiu Tao Sejak"
              value={formatDate(anggota.chiu_tao_sejak)}
            />
          ...
          <div style={{ display: "flex", gap: "10px", marginTop: "4px" }}>
            ...
            <button
              ...
              onMouseLeave={(e) =>
                (e.currentTarget.style.background = "rgba(200,100,100,0.08)")
              }
            >
              {deleting ? (
                <Loader2
    ...
    >
      <div
        style={{
          ...
          height: "1px",
          background: hovered
            ? `linear-gradient(90deg, transparent, ${warna}60, transparent)`
            : "transparent",
          transition: "all 0.3s",
        }}
      />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <div
            style={{
              ...
              height: "6px",
              borderRadius: "50%",
              background: anggota.status === "Aktif" ? "#7ec8a0" : "#c87e7e",
            }}
          />
          <span
            style={{
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
          ...
          <span style={{ fontSize: "12px", color: "#6b7191" }}>
            Baptis {formatDate(anggota.tanggal_baptis)}
          </span>
        </div>
  ...
  const FilterBtn = ({ label, active, onClick }) => (
    <button
      ...
      style={{
        ...
        fontSize: "12px",
        fontWeight: "500",
        border: `1px solid ${active ? "rgba(200,169,110,0.4)" : "#1e2235"}`,
        background: active ? "rgba(200,169,110,0.12)" : "transparent",
        color: active ? "#c8a96e" : "#454866",
        cursor: "pointer",
        ...
        >
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              placeholder="Cari nama atau nama baptis..."
              value={search}
