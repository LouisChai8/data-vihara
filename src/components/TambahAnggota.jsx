...
function SelectField({ label, field, options, placeholder, form, errors, onChange }) {
  return (
    <div>
      <label style={labelStyle}>{label}</label>
      <div style={{ position: 'relative' }}>
        ...
        >
          {placeholder && (
            <option value="" disabled style={{ color: '#454866' }}>{placeholder}</option>
          )}
          {options.map(o => (
            <option key={o} value={o} style={{ color: '#d0d4e8', background: '#12141f' }}>{o}</option>
          ))}
        </select>
...
export default function TambahAnggota({ onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!valid) return   // stay on step 4, errors show inline
    setLoading(true)
    try {
    ...
    4: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <SelectField
          label="Peran (Role) *"
          field="peran"
          options={['Administrator', 'Chiang Se', 'Tan Chu', 'Ciang Yen', 'Pan Shi Ren Yuan']}
        ...
        {/* Summary */}
        <div style={{ background: '#0d0f18', border: '1px solid #1e2235', borderRadius: '12px', padding: '18px', marginTop: '4px' }}>
          ...
          {[
            ...
            <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid #1a1d2e' }}>
              ...
              <span style={{ fontSize: '12px', color: v === '—' ? '#353858' : '#c9cdd8', fontWeight: '500', maxWidth: '60%', textAlign: 'right' }}>{v}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          <div style={{ padding: '24px 28px 0', background: 'linear-gradient(180deg,#161928 0%,#12141f 100%)', borderBottom: '1px solid #1a1d2e' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '22px' }}>
              ...
              <button
                ...
                style={{ width: '34px', height: '34px', borderRadius: '9px', background: '#1a1d2e', border: '1px solid #252840', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7191', flexShrink: 0, marginTop: '4px' }}
