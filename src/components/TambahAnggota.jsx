...
function Field({ label, field, type = 'text', placeholder = '', form, errors, onChange }) {
  return (
    <div>
      ...
      />
      {errors[field] && (
        <p style={{ fontSize: '11px', color: '#c87e7e', marginTop: '4px' }}>{errors[field]}</p>
      )}
    </div>
  )
}
      ...
      >
        ...
        >
          ...
          <div style={{ padding:'16px 28px 24px', borderTop:'1px solid #1a1d2e', display:'flex', justifyContent:'space-between', alignItems:'center', gap:'12px' }}>
            <button
              onClick={() => step > 1 ? setStep(s => s - 1) : onClose()} disabled={loading}
            ...
            {step < 4 ? (
              ...
              <button
                ...
                style={{ padding:'10px 22px', borderRadius:'10px', background: loading ? '#1e2235' : 'linear-gradient(135deg,#c8a96e,#a07840)', border:'none', color: loading ? '#454866' : '#0d0f18', fontSize:'13px', fontWeight:'600', cursor: loading ? 'not-allowed' : 'pointer', fontFamily:"'DM Sans', sans-serif", display:'flex', alignItems:'center', gap:'8px', minWidth:'140px', justifyContent:'center', boxShadow: loading ? 'none' : '0 4px 16px rgba(200,169,110,0.2)' }}
              >
                {loading
